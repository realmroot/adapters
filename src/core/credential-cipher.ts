...
export function createCredentialCipher(encodedKey: string): CredentialCipher {
  ...
  return {
    ...
    async open(value, context) {
      ...
      if (version !== 'v1' || !encodedIv || !encodedCiphertext) throw new TypeError('Credential ciphertext is invalid.')
...
function decodeBase64(value: string) {
  ...
  try {
    ...
    throw new TypeError('Credential encryption key is not valid base64.')
  }
}
