...
describe('adapter Worker configuration', () => {
  it('reads bindings and derives the canonical adapter origin from the request', () => {
    const environment = {
      ...
      GITHUB_API_ORIGIN: 'https://untrusted-github.example/',
      GITHUB_UPLOADS_ORIGIN: 'https://untrusted-uploads.example/',
      GITHUB_APP_ID: '123',
      GITHUB_PRIVATE_KEY: 'private-key',
      GITHUB_WEBHOOK_SECRET: 'github-webhook-secret-with-32-characters',
      LINEAR_API_ORIGIN: 'https://untrusted-linear-api.example/',
      LINEAR_AUTHORIZATION_ORIGIN: 'https://untrusted-linear.example/',
  ...
  it('rejects short webhook secrets', () => {
    const base = {
      REALMROOT_ISSUER: 'https://local.realmroot.dev/api/auth',
      REALMROOT_JWKS_URL: 'https://local.realmroot.dev/api/auth/jwks',
    }
